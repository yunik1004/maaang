...
{#if open}
	...
	<!-- 모달 -->
	<div
		class="fixed top-1/2 left-1/2 z-50 flex max-h-[90vh] w-[calc(100%-2rem)] max-w-lg -translate-x-1/2 -translate-y-1/2 flex-col border border-primary/40 bg-base-200 shadow-2xl"
	>
		<!-- 상단 장식선 -->
		<div
			class="h-0.5 w-full shrink-0 bg-gradient-to-r from-transparent via-primary to-transparent"
		></div>

		<div class="flex min-h-0 flex-col">
			<!-- 헤더 -->
			<div class="mb-4 flex shrink-0 items-center justify-between px-8 pt-8">
			...
			<!-- 탭 -->
			<div class="flex shrink-0 border-b border-primary/20 px-8">
				{#each ['interpreter', 'transpiler'] as const as t (t)}
					<button
			...
			<ul class="space-y-3 overflow-y-auto px-8 py-6">
				{#each tab === 'interpreter' ? interpRules : transpilerRules as entry (entry.title)}
					...
					{:else}
						<li class="border-l-2 border-primary/30 pl-4">
							<span class="text-[11px] tracking-widest text-base-content/40 uppercase"
								>{entry.title}</span
							>
							<code class="mt-1 block font-mono text-sm whitespace-pre text-primary/90"
								>{entry.example}</code
							>
						</li>
					{/if}
		...
		<!-- 하단 장식선 -->
		<div
			class="h-px w-full shrink-0 bg-gradient-to-r from-transparent via-primary/50 to-transparent"
		></div>
	</div>
{/if}
